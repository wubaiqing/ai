...
class AIContentService {
  ...
  buildTweetAnalysisPrompt(tweetsData) {
    ...
    return `请分析以下推文数据，提取有价值的科技资讯信息，生成一份标准 Markdown 格式的中文简报。
...
### 标题层级规范
- **主要话题**：使用 ## 二级标题格式（如：## 🚀 通义千问发布多款新模型）
- **子话题**：使用 ### 三级标题格式（如果需要进一步细分）
- **标题风格**：主标题应概括该主题的核心内容，建议格式为"公司名 + 主要动作"
- **图标使用**：在标题前添加合适的 emoji 图标增强可读性

### 列表和内容格式
- **主要内容**：使用标准 Markdown 无序列表格式（- 开头）
- **子项目**：使用缩进的无序列表格式（  - 开头，两个空格缩进）
- **链接格式**：统一使用标准 Markdown 链接格式 [消息来源](推文URL)
- **段落分隔**：在不同主题之间添加空行，确保排版清晰
- **重点突出**：重要信息使用 **粗体** 标记

### 内容组织要求
- **语言统一**：如果内容是英文或其他语言，请翻译成中文
- **价值筛选**：确保每条信息都有实际价值，避免重复或无意义的内容
- **详细描述**：每个条目包含详细描述和准确的消息来源链接
- **逻辑清晰**：内容层次分明，易于阅读和理解
...
- **Qwen3-Omni 全模态模型**：发布了首个原生端到端全模态 AI 模型，能在一个模型中统一处理文本、图像、音频和视频，官方称在多项音视频基准测试中达到 SOTA 水平。已开源 Qwen3-Omni-30B-A3B 系列模型。[消息来源](推文URL)

- **Qwen-Image-Edit-2509 图像编辑**：全新的图像编辑模型，支持多图像编辑、保持人脸和产品一致性、编辑文字内容及样式，并内置 ControlNet 支持。[消息来源](推文URL)

- **Qwen3-TTS-Flash 语音合成**：发布新的文本转语音（TTS）模型，支持 17 种音色、10 种语言及 9 种以上中文方言，首包响应时间仅 97 毫秒。[消息来源](推文URL)

## 🔥 OpenAI 发布新功能更新

- **具体功能描述**：详细的功能介绍和技术特点。[消息来源](推文URL)
