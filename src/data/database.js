...
async function storeTweetDataToSupabase(tweetDataArray) {
  ...
  try {
    ...
    if (insertionError) {
      Logger.error('[数据库] 存储推文数据失败', { error: insertionError.message });
      Logger.error('[数据库] 错误详情', { error: insertionError });
      throw new Error(`数据库存储失败: ${insertionError.message}`);
    }

    Logger.info(`[数据库] 推文数据存储成功，共 ${uniqueTweets.length} 条`);
    return { success: true, data: insertedData, count: uniqueTweets.length };
  } catch (error) {
    Logger.error('[数据库] 存储操作异常', { error: error.message });
    throw error;
  }
}
...
async function retrieveTweetDataFromSupabase(limit = 100, listId = null) {
  try {
    ...
    Logger.error('[数据库] 查询操作异常', { error: error.message });
    throw error;
  }
}
